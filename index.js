...
console.log(”[db] Table signals prete”);
}
...
} catch (e) { console.warn(”[telegram] Echec:”, e.message); }
}
...
let s = 0;
if (c.change1h > 0) { s += Math.min(40, (c.change1h / 5) * 40); } else { s -= 20; }
...
let s = 0;
if (c.change1h < 0) { s += Math.min(40, (Math.abs(c.change1h) / 5) * 40); } else { s -= 20; }
...
const res = await pool.query(“SELECT * FROM signals WHERE resolved=false AND created_at <= $1”, [cutoff]);
for (const entry of res.rows) {
const currentPrice = priceMap[entry.coin_id];
...
await pool.query(“UPDATE signals SET resolved=true,result=$1,exit_price=$2,pct_change=$3,pts=$4 WHERE id=$5”,
[result, String(currentPrice), String(pct), pts, entry.id]);
const sign = pct >= 0 ? “+” : “”;
console.log(”[cron] Resolu: “ + entry.symbol + “ “ + entry.direction + “ -> “ + result + “ “ + sign + pct.toFixed(2) + “% “ + pts + “pts”);
const emoji = result === “correct” ? “OK” : result === “incorrect” ? “XX” : “->”;
await sendTelegram(emoji + “ RESULTAT - “ + entry.symbol + “ “ + entry.direction.toUpperCase() + “\nEntree : $” + fmtPrice(entryPrice) + “ -> Sortie : $” + fmtPrice(currentPrice) + “\nVariation : “ + sign + pct.toFixed(2) + “%\nScore : “ + pts + “/10 pts”);
}
}
...
const res = await pool.query(“SELECT id FROM signals WHERE coin_id=$1 AND direction=$2 AND created_at>=$3 AND resolved=false LIMIT 1”, [coin.id, direction, since]);
if (res.rows.length > 0) return;
const now = new Date();
await pool.query(“INSERT INTO signals (coin_id,symbol,image,direction,entry_price,signal_score,created_at) VALUES($1,$2,$3,$4,$5,$6,$7)”,
[coin.id, coin.symbol, coin.image, direction, String(coin.price), score, now]);
console.log(”[cron] Signal: “ + coin.symbol + “ “ + direction + “ score=” + score + “ @ $” + coin.price);
const dir = direction === “long” ? “LONG” : “SHORT”;
await sendTelegram(dir + “ SIGNAL - “ + coin.symbol + “\nScore : “ + score + “/100\nPrix : $” + fmtPrice(coin.price) + “\nDate : “ + fmtDate(now) + “\nResolution dans 30min”);
}

let isRunning = false;

async function runCycle() {
if (isRunning) { console.log(”[cron] Skip”); return; }
isRunning = true;
...
const tradeable = coins.filter(function(c) { return c.volume > 3000000; });
const scored = tradeable.map(function(c) { return Object.assign({}, c, { lScore: scoreForLong(c), sScore: scoreForShort(c) }); });
const bestLong = scored.slice().sort(function(a,b) { return b.lScore - a.lScore; })[0];
const bestShort = scored.slice().sort(function(a,b) { return b.sScore - a.sScore; })[0];
...
app.get(”/api/test-telegram”, async function(_req, res) { await sendTelegram(“Test TRADEYE OK - “ + fmtDate(new Date())); res.json({ success: true }); });

app.get(”/api/signals/history”, async function(_req, res) {
try {
const since = new Date(Date.now() - 30*24*60*60*1000);
const r = await pool.query(“SELECT * FROM signals WHERE created_at>=$1 ORDER BY created_at DESC”, [since]);
res.json({ success: true, data: r.rows });
} catch(e) { res.status(500).json({ success: false, error: e.message }); }
});

app.get(”/api/signals/perfo”, async function(_req, res) {
try {
const since = new Date(Date.now() - 7*24*60*60*1000);
const r = await pool.query(“SELECT * FROM signals WHERE resolved=true AND created_at>=$1”, [since]);
const correct = r.rows.filter(function(x){return x.result===“correct”;}).length;
const incorrect = r.rows.filter(function(x){return x.result===“incorrect”;}).length;
const neutral = r.rows.filter(function(x){return x.result===“neutral”;}).length;
const withPts = r.rows.filter(function(x){return x.pts!=null;});
const score7d = withPts.length ? withPts.reduce(function(s,x){return s+x.pts;},0)/withPts.length : null;
const winRate = r.rows.length ? (correct/r.rows.length)*100 : null;
res.json({ success: true, data: { correct, incorrect, neutral, total: r.rows.length, score7d, winRate }});
} catch(e) { res.status(500).json({ success: false, error: e.message }); }
});

app.get(”/api/signals/export”, async function(*req, res) {
try {
const since = new Date(Date.now() - 30*24*60*60*1000);
const r = await pool.query(“SELECT * FROM signals WHERE created_at>=$1 ORDER BY created_at DESC”, [since]);
const resolved = r.rows.filter(function(x){return x.resolved;});
const pending = r.rows.filter(function(x){return !x.resolved;});
const correct = resolved.filter(function(x){return x.result===“correct”;}).length;
const incorrect = resolved.filter(function(x){return x.result===“incorrect”;}).length;
const neutral = resolved.filter(function(x){return x.result===“neutral”;}).length;
const withPts = resolved.filter(function(x){return x.pts!=null;});
const score = withPts.length ? (withPts.reduce(function(s,x){return s+x.pts;},0)/withPts.length).toFixed(1) : “-”;
const wr = resolved.length ? Math.round((correct/resolved.length)*100)+”%” : “-”;
const SEP = “==================================================\n”;
let txt = SEP + “  TRADEYE - HISTORIQUE (30 JOURS)\n  Exporte le “ + fmtDate(new Date()) + “\n” + SEP + “\n”;
txt += “SCORE 7J : “ + score + “/10 | WIN RATE : “ + wr + “\n”;
...
console.log(”[cron] Demarrage 3min”);
runCycle();
setInterval(function() { runCycle(); }, 180000);
}
